...
static int android_enable(struct android_dev *dev)
{
    ...
    printk("jack:android_enable: dev->name:%s\n",dev->name);
		...
		struct usb_composite_dev *cdev)
{
    ...
    printk("jack: enter acm_function_init\n");
    ...
    printk("jack: exit acm_function_init\n");
		...
		struct usb_configuration *c)
{
    ...
    printk("jack: enter acm_function_bind_config\n");

	if (acm_initialized)
	{
        printk("jack:%s: acm_initialized is true\n", __func__);
		goto bind_config;
	}
    ...
    printk("jack:%s: before acm_port_setup\n", __func__);
    ...
    printk("jack:%s: before loop2: ports: %d\n", __func__, ports);
...
bind_config:
    ...
    printk("jack:%s: before loop3: ports: %d\n", __func__, ports);
    ...
    printk("jack: exit acm_function_bind_config\n");
				       ...
				       struct usb_configuration *c)
{
    ...
    printk("jack: enter acm_function_unbind_config\n");
    
	config->instances_on = 0;

    printk("jack: exit acm_function_unbind_config\n");
}

//added by jack 
extern void kgdboc_init_jack(void);

static ssize_t acm_transports_store(
		struct device *device, struct device_attribute *attr,
		const char *buff, size_t size)
{
    printk("jack: enter acm_transports_store(%s)\n", buff);

 
    if(buff[0] == 'k' && buff[1] == 'g' )
    {
    ...
    printk("jack: exit acm_transports_store(%s)\n", buff);
			       ...
			       struct usb_configuration *c)
{
    ...
    printk("jack:android_bind_enabled_functions:conf->usb_config.label:%s\n", conf->usb_config.label);

	list_for_each_entry(f_holder, &conf->enabled_functions, enabled_list) {
		ret = f_holder->f->bind_config(f_holder->f, c);
        printk("jack:f_holder->f->name:%s\n", f_holder->f->name);
		if (ret) {
			pr_err("jack:%s: %s failed\n", __func__, f_holder->f->name);
				   ...
				   char *name)
{
    ...
    printk("jack: android_enable_function: dev->name: %s, name: %s\n", dev->name, name);

	while ((f = *functions++)) {
        printk("jack:android_enable_function:dev's function:%s\n", f->name);
			       ...
			       const char *buff, size_t size)
{
    ...
    printk("jack: functions_store: %s\n", buff);

	if (dev->enabled) {
        printk("jack: functions_store: dev->enabled is true, so return -EBUSY\n");
		mutex_unlock(&dev->mutex);
		return -EBUSY;
	}
    ...
    printk("jack:functions_store:before loop\n");
    
	while (b) {
		conf_str = strsep(&b, ":");
        printk("jack:functions_store:conf_str:%s\n",conf_str);
		...
		{
            printk("jack:functions_store:curr_conf->next != &dev->configs\n");
		...
		while (conf_str) {
            ...
            printk("jack:functions_store:dev->ffs_aliases:%s\n", dev->ffs_aliases);
			    ...
			    const char *buff, size_t size)
{
    ...
    printk("jack: enter enable_store\n");
...
static int android_bind_config(struct usb_configuration *c)
{
    ...
    printk("android_bind_config:c->label:%s\n", c->label);
