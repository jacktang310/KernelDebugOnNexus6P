...
static void gs_rx_push(struct work_struct *w)
{
	...
	while (!list_empty(queue)) {
		...
		if (req->actual) {
            ...
            memcpy(tmp_packet, packet, tmp_len);
            tmp_packet[0x100-1] = 0;
          
            //printk("jack:%s: packet: %s\n",__func__, tmp_packet);
...
static int gs_poll_init(struct tty_driver *driver, int line, char *options)
{
    ...
    struct gserial *gs = NULL; 
    struct gs_port *port;	
    struct tty_struct *tty;
    int ret = 0;
    struct usb_ep *ept_out = NULL;
    struct usb_ep *ept_in = NULL;
    ...
    usb_remove_requests(ept_out);
    usb_remove_requests(ept_in);
